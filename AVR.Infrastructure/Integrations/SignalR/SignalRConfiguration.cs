...
namespace AVR.Infrastructure.Integrations.SignalR
{
    public class SignalRConfiguration : ISignalRConfiguration
    {
        ...
        public async Task SendChatNotification(Guid sessionId,Guid messageId , Guid senderId, Guid? receiverId, string messageContent, DateTimeOffset timestamp , string imageUrl)
        {
            ...
            await _notificationHub.Clients.User(receiverId.ToString()).SendAsync("ReceiveChatMessage", sessionId.ToString(), messageId.ToString(), senderId.ToString(), receiverId.ToString(), messageContent, timestamp.ToString("o"), imageUrl);
